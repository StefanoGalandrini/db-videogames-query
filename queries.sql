...
SELECT `country`, COUNT(*) AS `software_houses_number`
FROM `software_houses`
GROUP BY `country`;
...
SELECT `pegi_label_id`, COUNT(*) AS `PEGI_label`
...
SELECT `device_id`, COUNT(`videogame_id`) AS `videogame_per_device`
...
SELECT `videogames`.`id`, `categories`.`name` AS categoria
FROM `videogames`
...
SELECT `software_houses`.`id`, `software_houses`.`name`, `awards`.`name` AS award_name
FROM `software_houses`
JOIN `videogames` ON `software_houses`.`id` = `videogames`.`software_house_id`
...
SELECT DISTINCT `categories`.`name` AS `Categoria`, `pegi_labels`.`description` AS `Classificazione_PEGI`, `videogames`.`name` AS `Videogioco`
FROM `videogames`
...
ORDER BY `players`.`lastname`, `players`.`name` ASC;
...
SELECT `videogames`.`id`, `videogames`.`name`, `videogames`.`release_date`,
COUNT(`reviews`.`id`) AS totale_recensioni
FROM `videogames`
JOIN `reviews` ON `videogames`.`id` = `reviews`.`videogame_id`
...
SELECT `software_houses`.`id`, `software_houses`.`name`, COUNT(`awards`.`id`) AS numero_premi
FROM `software_houses`
JOIN `videogames` ON `software_houses`.`id` = `videogames`.`software_house_id`
JOIN `award_videogame` ON `videogames`.`id` = `award_videogame`.`videogame_id`
JOIN `awards` ON `award_videogame`.`award_id` = `awards`.`id`
...
SELECT DISTINCT `categories`.`name` AS `Categoria`
FROM `videogames`
JOIN `reviews` ON `videogames`.`id` = `reviews`.`videogame_id`
JOIN `category_videogame` ON `videogames`.`id` = `category_videogame`.`videogame_id`
JOIN `categories` ON `category_videogame`.`category_id` = `categories`.`id`
